...
TEST_F(ParserTest, RealWorldExample3) {
    // Example from README
    auto expr = parse("not @deprecated and (@fast or @smoke)");
    
    EXPECT_TRUE(expr->evaluate({"@fast"}));
    EXPECT_TRUE(expr->evaluate({"@smoke"}));
    EXPECT_FALSE(expr->evaluate({"@deprecated", "@fast"}));
}
